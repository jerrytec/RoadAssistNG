import { Car, Bike, Footprints, ArrowRight, ArrowLeftRight, ExternalLink, AlertTriangle, X, type LucideIcon } from "lucide-react";
...
const MODES: { value: TravelMode; Icon: LucideIcon; label: string }[] = [
  { value: "DRIVE",       Icon: Car,        label: "Drive" },
  { value: "TWO_WHEELER", Icon: Bike,       label: "Bike" },
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-3 mb-3 animate-fade-in">
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <button onClick={onClose} className="text-muted-foreground hover:text-foreground shrink-0 transition-colors" aria-label="Close directions">
      ...
      <div className="flex items-center gap-1.5 mb-2 flex-wrap">
          ...
          {MODES.map((m) => {
            ...
            return (
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-[11px] font-semibold transition-colors inline-flex items-center gap-1 ${
                  active ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                }`}
                aria-pressed={active}
                aria-label={m.label}
              >
                <ModeIcon className="w-3.5 h-3.5" aria-hidden="true" /> {m.label}
              </button>
            );
          })}
        </div>
        <button
          onClick={onSwap}
          className="px-2.5 py-1 rounded-full text-[11px] font-semibold border border-border hover:bg-muted hover:border-primary/40 transition-colors inline-flex items-center gap-1"
          aria-label="Swap origin and destination"
          title="Swap origin and destination"
        >
          <ArrowLeftRight className="w-3.5 h-3.5" aria-hidden="true" /> Swap
        </button>
        {externalHref && (
          <a
            href={externalHref}
            target="_blank"
            rel="noreferrer"
            className="ml-auto text-[11px] font-semibold text-primary hover:underline inline-flex items-center gap-1"
          >
